...
| **Các bước thực hiện**   | 1. User A mở trang đấu giá.<br>2. Nhập giá `1.100.000 VND`.<br>3. Gửi yêu cầu đặt giá.<br>4. Kiểm tra WebSocket frame.<br>5. Kiểm tra log `la-ws-handler`.<br>6. Kiểm tra message được gửi vào SQS FIFO.<br>7. Kiểm tra `la-bid-processor` xử lý message.<br>8. Kiểm tra DynamoDB.<br>9. Kiểm tra message broadcast và giao diện. |
...
| **Các bước thực hiện**   | 1. Gửi giá `1.100.000 VND`.<br>2. Gửi tiếp giá `1.000.000 VND` bằng Request ID khác.<br>3. Kiểm tra kết quả từng yêu cầu.<br>4. Kiểm tra DynamoDB và lịch sử đặt giá.<br>5. Kiểm tra broadcast.                             |
...
| **Các bước thực hiện**   | 1. User A gửi giá `1.150.000 VND`.<br>2. Kiểm tra quá trình xử lý.<br>3. Kiểm tra response.<br>4. Kiểm tra DynamoDB.<br>5. Kiểm tra các user khác có nhận cập nhật hay không.                                                        |
...
| **Các bước thực hiện**   | 1. Mở phiên chưa bắt đầu.<br>2. Gửi một mức giá hợp lệ về mặt số tiền.<br>3. Kiểm tra response.<br>4. Kiểm tra trạng thái và thời gian phiên trong DynamoDB.<br>5. Kiểm tra lịch sử bid.    |
...
| **Các bước thực hiện**   | 1. Mở phiên đã kết thúc.<br>2. Gửi một mức giá cao hơn current price.<br>3. Kiểm tra response.<br>4. Kiểm tra DynamoDB.<br>5. Kiểm tra winner và bid history.     |
...
| **Các bước thực hiện**   | 1. Thử đặt giá khi chưa đăng nhập.<br>2. Thử bằng token hết hạn.<br>3. Thử bằng token sai chữ ký.<br>4. Thử gửi `userId` giả trong message.<br>5. Kiểm tra SQS, DynamoDB và logs.                           |
...
| **Các bước thực hiện**   | 1. Đăng nhập bằng User C.<br>2. Gửi yêu cầu đặt giá đến Auction Active.<br>3. Kiểm tra authorization result.<br>4. Kiểm tra SQS và processor log.<br>5. Kiểm tra DynamoDB và broadcast. |
...
| **Các bước thực hiện**   | 1. Gửi bid hợp lệ với `requestId=bid-request-008`.<br>2. Chờ yêu cầu được xử lý thành công.<br>3. Gửi lại chính xác request đó.<br>4. Nếu có thể, mô phỏng SQS redelivery.<br>5. Kiểm tra bid history, current price và broadcast. |
...
| **Các bước thực hiện**   | 1. User A đặt `1.100.000 VND`.<br>2. Chờ kết quả thành công.<br>3. User B đặt `1.200.000 VND`.<br>4. User A đặt `1.300.000 VND`.<br>5. Kiểm tra thứ tự message trong SQS FIFO.<br>6. Kiểm tra DynamoDB và các event broadcast. |
...
| **Các bước thực hiện**   | 1. Ghi nhận highest bidder ban đầu.<br>2. User A gửi bid hợp lệ.<br>3. Kiểm tra highest bidder.<br>4. User B gửi bid cao hơn.<br>5. Kiểm tra lại highest bidder.<br>6. Gửi một bid không hợp lệ từ User A.<br>7. Kiểm tra dữ liệu lần cuối. |
...
| **Các bước thực hiện**   | 1. Thực hiện nhiều bid hợp lệ.<br>2. Truy vấn lịch sử theo Auction ID hoặc Item ID.<br>3. Kiểm tra User ID, amount, timestamp, request ID và sequence.<br>4. Kiểm tra thứ tự hiển thị.<br>5. Đối chiếu với processor logs và current price.              |
...
| **Các bước thực hiện**   | 1. Mở Room A bằng User A và User B.<br>2. Mở Room B bằng User C.<br>3. User A đặt giá hợp lệ trong Room A.<br>4. Quan sát message tại ba cửa sổ.<br>5. Kiểm tra `la-broadcast` logs.<br>6. Kiểm tra danh sách connection được truy vấn. |
...
| **Các bước thực hiện**   | 1. Ghi nhận giá hiển thị ở hai cửa sổ.<br>2. User A đặt giá hợp lệ.<br>3. Không tải lại trang.<br>4. Quan sát giá mới ở cả hai cửa sổ.<br>5. Kiểm tra highest bidder, bid history và thông báo giao diện.<br>6. Kiểm tra browser console và Network tab. |
...
- WebSocket frame gửi yêu cầu đặt giá.
- WebSocket frame trả kết quả.
- Hai hoặc ba cửa sổ trình duyệt cùng hoạt động.
- Giá hiển thị trước và sau khi đặt.
- Lịch sử đặt giá trên frontend.
- CloudWatch Logs của `la-ws-handler`.
- SQS message metadata đã che thông tin nhạy cảm.
- CloudWatch Logs của `la-bid-processor`.
- DynamoDB current price trước và sau.
- DynamoDB highest bidder trước và sau.
- Bid history records.
- CloudWatch Logs của `la-broadcast`.
- Kết quả xử lý request trùng.
- Kết quả conditional write khi có cạnh tranh.
- Request ID dùng để liên kết toàn bộ luồng.

Ví dụ cách đặt tên:

```
Hình 5.5.6.1: User A gửi yêu cầu đặt giá hợp lệ trong BID-01
Hình 5.5.6.2: Message được xử lý bởi la-bid-processor
Hình 5.5.6.3: Giá và highest bidder được cập nhật trong DynamoDB
Hình 5.5.6.4: Hai User nhận sự kiện BID_ACCEPTED trong BID-12
Hình 5.5.6.5: Frontend hiển thị giá mới không cần tải lại trang
Hình 5.5.6.6: Request trùng chỉ tạo một bid history trong BID-08
```
...
- Yêu cầu hợp lệ đi qua đầy đủ luồng.
- Yêu cầu không hợp lệ không làm thay đổi dữ liệu.
- Current price được cập nhật chính xác.
- Highest bidder được xác định chính xác.
- Bid history được lưu đúng và không trùng.
- Request trùng chỉ được áp dụng một lần.
- Thứ tự bid trong cùng phiên được bảo toàn.
- Broadcast chỉ gửi đến đúng phòng.
- Frontend cập nhật mà không cần tải lại trang.
- Không có token hoặc dữ liệu nhạy cảm trong log và bằng chứng.

Test case phải được đánh dấu `FAIL` khi:

- Bid không hợp lệ vẫn làm thay đổi giá.
- Bid hợp lệ không được lưu.
- Một bid thấp hơn ghi đè bid cao hơn.
- Highest bidder không khớp với bid cao nhất.
- Một request trùng tạo hai bản ghi lịch sử.
- Bid history và current price không nhất quán.
- Phiên chưa bắt đầu hoặc đã kết thúc vẫn nhận bid.
- User chưa xác thực hoặc không có quyền vẫn đặt giá được.
- User ngoài phòng nhận dữ liệu của phòng khác.
- Một connection lỗi làm toàn bộ broadcast thất bại.
- Frontend chỉ cập nhật sau khi tải lại trang.
- Logs hoặc bằng chứng làm lộ token.

Test case được đánh dấu `BLOCKED` khi:

- Route đặt giá chưa được triển khai.
- `la-ws-handler` chưa gửi message vào SQS.
- SQS FIFO chưa tồn tại.
- `la-bid-processor` chưa được kết nối với SQS.
- DynamoDB chưa có cấu trúc lưu current bid hoặc bid history.
- Cơ chế idempotency chưa được triển khai.
- `la-broadcast` chưa tồn tại.
- Frontend chưa xử lý sự kiện bid.
- Không có quyền kiểm tra SQS, DynamoDB hoặc CloudWatch Logs.
